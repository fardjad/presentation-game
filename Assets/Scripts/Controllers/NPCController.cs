    ...
    public class NPCController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            this.UpdateAsObservable()
                .Timestamp()
                .Sample(Observable.Interval(TimeSpan.FromMilliseconds(500)))
                .Subscribe(ts =>
                {
                    zmqipcManager.Send(new NetAction
                    {
                        Type = ActionType.GAMEOBJECT_STATUS,
                        Payload = new GameObjectStatusPayload
                        {
                            Id = Id,
                            StoppingDistance = m_Agent.stoppingDistance,
                            RemainingDistance = m_Agent.remainingDistance,
        ...
        private void Move(Vector3 move, bool crouch, bool jump)
        {
            ...
            m_ForwardAmount = move.z * 0.5f; // force walking
        ...
        {
            if (m_Animator.GetCurrentAnimatorStateInfo(0).IsName("Kicking"))
            {
        ...
        public void Update()
        {
            if (m_Agent.destination != Target.position)
            {
            ...
            if (m_Agent.hasPath)
            {
                ...
                points.AddRange(m_Agent.path.corners);

                if (points.Count >= 2)
                {
                    for (var i = 0; i < points.Count - 1; i++)
                    {
                        Debug.DrawLine(points[i], points[i + 1], Color.green, Time.deltaTime, false);
                    }
                }
            }

            Move(
                m_Agent.remainingDistance > m_Agent.stoppingDistance ? m_Agent.desiredVelocity : Vector3.zero,
                false,
                false
            );
        }
